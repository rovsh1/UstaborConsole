...
use Api\Model\Page\Content as PageContent;
use Html\Page\Page as HtmlPage;
use Controller\Url;
...
class Controller_Action{
	...
	protected function set($name, $param) {
		$this->dataManager[$name] = $param;
	}
	
	public function has($name) {
		return array_key_exists($name, $this->dataManager);
	}
	
	public function get($name) {
		return $this->has($name) ? $this->dataManager[$name] : null;
	}
	...
	protected function initUrl() {
		...
		switch (count($partsTemp)) {
			case 0:
				...
				//break;
			case 1:
				$action = str_replace('-', '_', $partsTemp[0]);
		...
		if (ServiceManager::has('page')) {
			$page = ServiceManager::get('page');
			$content = new PageContent();
			if ($content->findByName('default')) {
	...
	protected function checkPath() {
		...
		if (!$result && ServiceManager::has('page') && !ServiceManager::get('page')->isEmpty()) {
			$request->setActionName('index');
			$result = true;
		}
		return $result;
	...
	protected function setPageContent($metadata) {
		$htmlPage = $this->page;
		$htmlHead = $htmlPage->getHead();
		if (!$metadata->isEmpty()) {
			$htmlPage->setH1($metadata->h1);
			$htmlPage->setText($metadata->text);
			$htmlHead->setTitle($metadata->title);
			$htmlHead->setKeywords($metadata->keywords);
			$htmlHead->setDescription($metadata->description);
			$htmlHead->fromString($metadata->getHeadExtended());
		}
	}
