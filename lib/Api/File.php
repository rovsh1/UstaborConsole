...
class File extends \File{
	...
	protected static function checkPath($guid) {
		...
		while (!empty($paths)) {
			...
			if (!is_dir($dir)) {
				mkdir($dir, AppConfig::get('filesystem.chmod.dir', 0770));
				self::chmod($dir);
			}
		}
		return true;
	...
	protected static function chmod($filename) {
		if (is_file($filename) && ($mode = AppConfig::get('filesystem.chmod.file'))) {
			chmod($filename, $mode);
		}
		if (($group = AppConfig::get('filesystem.group')))
			chgrp($filename, $group);
		if (($user = AppConfig::get('filesystem.user')))
			chown($filename, $user);
	}
